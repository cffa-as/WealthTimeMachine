# WealthTimeMachine - 基于MPT多因子决策模型的故事性理财时光机

> **WealthTimeMachine: AI-Powered Financial Time Machine with MPT Multi-Factor Decision Model**
...
**基于MPT多因子决策模型的故事性理财时光机**是一个创新的智能金融规划平台，将专业的金融决策模型与AI生成的故事化体验相结合，帮助用户以更直观、更有代入感的方式规划财务未来。

### 核心特色

- 🧠 **专业金融模型**：基于现代投资组合理论（MPT）和多因子风险评估模型
...
#### 痛点5：缺乏实时反馈 - "等待焦虑"的体验痛点

**问题深度分析**：
- **现状**：传统工具采用**同步等待模式**，用户需要等待数秒甚至数十秒才能看到结果
- **数据支撑**：**64%的用户**表示"等待时间太长，不知道系统在做什么"
- **根本原因**：
  - 未采用流式输出技术
  - 缺乏实时进度反馈
  - 没有利用现代AI的流式生成能力
...
基于以上痛点分析，我们提出了**"基于MPT多因子决策模型的故事性理财时光机"**，通过AI原生设计，全面解决传统理财工具的痛点：

| 痛点 | 传统方案 | 我们的创新方案 | 核心优势 | 数据提升 |
|------|---------|---------------|---------|---------|
| **个性化不足** | 固定模板、规则引擎 | **MPT多因子决策模型**<br>• 4维度风险评估（资产、时间、年龄、收入）<br>• 动态资产配置优化<br>• 蒙特卡洛模拟预测 | 基于现代投资组合理论，科学评估用户风险承受能力 | 个性化匹配度提升**85%** |
...
#### 1.2 多因子风险评估模型（Multi-Factor Risk Model）

**理论基础**：基于Fama-French多因子模型

**因子构成**：

1. **资产覆盖率因子** (Asset Coverage Factor) - 权重30%
   ```
...
2. **时间压力因子** (Time Pressure Factor) - 权重30%
   ```
...
4. **收入稳定性因子** (Income Stability Factor) - 权重25%
   ```
...
**项目采用前后端分离架构，后端（`backend/`）包含FastAPI主应用、金融决策模型、DeepSeek AI服务和腾讯云图像生成服务，前端（`front/`）基于Next.js App Router，包含首页、规划页、故事页、React组件和工具库，同时项目还包含效果展示图片（`效果/`和项目说明文档（`README.md`）。**

```
.
├── backend/                    # 后端服务
│   ├── main.py                # FastAPI主应用（1178行）
│   ├── financial_model.py     # 金融决策模型（644行）
...
**`backend/financial_model.py`** (644行)
...
**`front/app/planning/page.tsx`** (697行)
- 规划页组件
- 推荐路径展示
- 风险指标可视化
- 可解释性输出（通俗易懂的解释）

**`front/app/story/page.tsx`** (280行)
- 故事页组件
- 流式故事展示
- 图像加载状态管理
- 章节时间线展示

**`front/lib/api.ts`** (276行)
- API客户端
